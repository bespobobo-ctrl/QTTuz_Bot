...
async function update() {
    const { error } = await supabase.from('system_config').update({ value: '12.2 WAREHOUSE-NETO-PRO' }).eq('key', 'app_version');
    if (error) console.error(error);
    else console.log('Updated DB version to 12.2 WAREHOUSE-NETO-PRO');
}
update();
