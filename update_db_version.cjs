...
async function update() {
    const { error } = await supabase.from('system_config').update({ value: '11.9 WAREHOUSE-RADAR' }).eq('key', 'app_version');
    if (error) console.error(error);
    else console.log('Updated DB version to 11.9 WAREHOUSE-RADAR');
}
update();
