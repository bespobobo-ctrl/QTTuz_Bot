...
async function update() {
    const { error } = await supabase.from('system_config').update({ value: '11.4 WAREHOUSE-ULTRA' }).eq('key', 'app_version');
    if (error) console.error(error);
    else console.log('Successfully updated DB version to 10.5 WAREHOUSE-ULTRA');
}
update();
