...
async function update() {
    const { error } = await supabase.from('system_config').update({ value: '12.9 ULTRA STABLE' }).eq('key', 'app_version');
    if (error) console.error(error);
    else console.log('Updated DB version to 12.9 ULTRA STABLE');
}
update();
