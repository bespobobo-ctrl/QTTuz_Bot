const { createClient } = require('@supabase/supabase-js');

const SUPABASE_URL = "https://woonyxwygwwnhnghqihu.supabase.co";
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Indvb255eHd5Z3d3bmhuZ2hxaWh1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY2NTk3NTUsImV4cCI6MjA5MjIzNTc1NX0.JmxloO9JSLkrJXY_S1WmWlIecSHqCzq1idygtHhlxwU";
const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);

async function checkBichuv() {
    const { data, error } = await supabase.from('department_logins').select('*').eq('dept_id', 'bichuv');
    if (data && data.length > 0) {
        console.log('Bichuv Login:', data[0]);
    } else {
        console.log('No login for Bichuv. Creating one...');
        await supabase.from('department_logins').insert({
            dept_id: 'bichuv',
            login_name: 'bichuv',
            password: '123'
        });
        console.log('Created: bichuv / 123');
    }
}
checkBichuv();
