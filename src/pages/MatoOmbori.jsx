...
export default function MatoOmboriPanel({ tab, data, load, showMsg }) {
    ...
    const renderStock = () => {
        // Faqat BRUTO statusli rulonlari bor yoki hali ruloni yo'q partiyalar
        const brutoBatches = batches.filter(batch => {
            const batchRolls = rolls.filter(r => String(r.batch_id) === String(batch.id));
            if (batchRolls.length === 0) return true; // Yangi partiya, hali rulon qo'shilmagan
            return batchRolls.some(r => r.status === 'BRUTO'); // Kamida bitta BRUTO rulon bor
        });

        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <h2 style={{ marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Package color="#FFAB40" /> Bruto Matolar ({brutoBatches.length} ta)
                </h2>

                {brutoBatches.length === 0 ? (
                    <div style={{ ...S.card, textAlign: 'center', opacity: 0.5, padding: 40 }}>Barcha partiyalar tekshirilgan! Bruto matolar yo'q.</div>
                ) : (
                    brutoBatches.map(batch => {
                        const batchRolls = rolls.filter(r => String(r.batch_id) === String(batch.id));
                    ...
                    <div style={{ display: 'grid', gap: 12 }}>
                        {(data.whLog || [])
                            .filter(l => {
                                ...
                                const type = String(l.action_type).trim().toUpperCase();
                                if (histFilter === 'all') return true;
                                if (histFilter === 'kirim') return type.includes('KIRIM');
                                if (histFilter === 'inspect') return ['INSPECTION_START', 'KONTROLDAN_OTDI', 'BRAK', 'WEIGHT_NETO', 'VERDICT_CONFIRMED', 'WEIGHT_BRUTO'].includes(type);
                                if (histFilter === 'delete') return type.includes('DELETE');
                                return true;
                            })
                            .map((l, idx) => {
                        ...
                        {(data.whLog || []).length === 0 && (
                            <div style={{ textAlign: 'center', padding: 100, opacity: 0.2 }}>
                                <History size={48} style={{ marginBottom: 15 }} />
                                <div>Tarix hali bo'sh</div>
                            </div>
                        )}
                    </div>
                </motion.div>
